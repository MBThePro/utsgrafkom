...
function multiplyMatrices(a, b) {
    ...
    if (!isValidMatrix(a) || !isValidMatrix(b) || !areCompatibleDimensions(a, b)) {
...
class MyObject {
	...
	r+=this.r;
	// console.log(PHI,theta,r);
	    var rotationMatrixX = [ // x
        [1, 0, 0],
        [0, Math.cos(PHI), -Math.sin(PHI)],
        [0, Math.sin(PHI), Math.cos(PHI)]
    ];

    var rotationMatrixY = [ // y
        [Math.cos(theta), 0, Math.sin(theta)],
        [0, 1, 0],
        [-Math.sin(theta), 0, Math.cos(theta)]
    ];

    var rotationMatrixZ = [ // z
    ...
    function rotateChildren(parent, parentOriginalMatrix) {
        for (var i = 0; i < parent.child.length; i++) {
            ...
            var rotatedTranslation = matrixVectorMultiply(rotationMatrix, translation);

            // Update the translation of the child object while preserving its original rotation
            parent.child[i].MOVEMATRIX[12] = parentOriginalMatrix[12] + rotatedTranslation[0];
            parent.child[i].MOVEMATRIX[13] = parentOriginalMatrix[13] + rotatedTranslation[1];
            parent.child[i].MOVEMATRIX[14] = parentOriginalMatrix[14] + rotatedTranslation[2];
